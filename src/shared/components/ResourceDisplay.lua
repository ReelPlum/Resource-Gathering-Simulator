...
function ResourceDisplay:init()
	...
	self.Value, self.SetValue = roact.createBinding(ClientController.Cache.Resources[self.props.Resource] or 0)
...
function ResourceDisplay:didMount()
	...
	self.Janitor:Add(ClientController.Signals.ResourcesChanged:Connect(function()
		self.SetValue(ClientController.Cache.Resources[self.props.Resource])
	end))
end
